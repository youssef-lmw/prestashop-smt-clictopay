...
class Clictopay extends PaymentModule
{
    ...
    public function install()
    {
        ...
        if (!Configuration::updateValue('SMT_SANDBOX', $this->SMT_SANDBOX)
            ...
            || !Configuration::updateValue('SMT_USERNAME', $this->SMT_USERNAME)
            || !Configuration::updateValue('SMT_PASSWORD', $this->SMT_PASSWORD)
        ) {
            return false;
        }
    ...
    public function uninstall()
    {
        ...
        if (!Configuration::deleteByName('SMT_SANDBOX')
            ...
            || !Configuration::deleteByName('SMT_USERNAME')
            || !Configuration::deleteByName('SMT_PASSWORD')
        ) {
            return false;
        }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submit' . $this->name)) {
            ...
            $smt_username = strval(Tools::getValue('SMT_USERNAME'));
            $smt_password= strval(Tools::getValue('SMT_PASSWORD'));
            ...
            if (!$smt_id
                ...
                || !Validate::isGenericName($smt_username)
            )
                $output .= $this->displayError($this->l('Invalid Configuration value'));
            else {
                Configuration::updateValue('SMT_ID', $smt_id);
                Configuration::updateValue('SMT_USERNAME', $smt_username);
                Configuration::updateValue('SMT_PASSWORD', $smt_password);
    ...
    public function displayForm()
    {
        ...
        $fields_form[0]['form'] = [
            ...
            'input' => [
                [
                    ...
                    'name' => 'SMT_USERNAME',
                    'placeholder' => $this->l('Terminal USERNAME provided by SMT'),
                    'size' => 20,
                    'required' => true
                ],
                [
                    ...
                    'name' => 'SMT_PASSWORD',
                    'placeholder' => $this->l('Terminal PASSWORD provided by SMT'),
                    'size' => 20,
                    'required' => true
                ],
                [
        ...
        // Load current value
        $helper->fields_value['SMT_USERNAME'] = Configuration::get('SMT_USERNAME');
        $helper->fields_value['SMT_PASSWORD'] = Configuration::get('SMT_PASSWORD');
        ...
        $payment_options->setCallToActionText($action_text)
            ...
            ->setInputs([
                ...
                'userName' => [
                    'name' => 'userName',
                    'type' => 'hidden',
                    'value' => Configuration::get('SMT_USERNAME'),
                ],
                'password' => [
                    'name' => 'password',
                    'type' => 'hidden',
                    'value' => Configuration::get('SMT_PASSWORD'),
                ],
